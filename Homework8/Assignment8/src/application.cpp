...
void Application::init() {
  ...
  ropeEuler = new Rope(Vector2D(0, 200), Vector2D(-400, 200), 16, config.mass,
                       config.ks, {0});
  ropeEuler_SimiImplicit = new Rope(Vector2D(0, 200), Vector2D(-400, 200), 16, config.mass,
                       config.ks, {0});
...
void Application::render() {
  ...
  for (int i = 0; i < config.steps_per_frame; i++) {
    ...
    ropeEuler_SimiImplicit->simulateEuler_SimiImplicit(1 / config.steps_per_frame, config.gravity);
